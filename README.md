...
- **Planner** — a monthly calendar, and now the app's home screen. Each day cell has a "+" to quickly add planned foods/meals (defaulting to Breakfast/Lunch/Dinner based on the time of day), with an inline "add a new food" flow if it isn't in your pantry yet, and shows a colour-coded macro total (calories + P/C/F) at the bottom of the cell. On wider screens the whole calendar scales up (bigger cells, bigger text) instead of staying capped at a small fixed size; on narrow screens each day cell holds its height steady and scrolls internally if a busy day doesn't fit. Clicking a day opens the full day view — a calorie ring, protein/carb/fat progress bars against your daily goals, and Breakfast/Lunch/Dinner/Snack sections to add or remove entries. Daily goals are editable via "Goals" in the header.
...
- **Shopping list** — a deduplicated checklist of every ingredient needed for what's logged today or in the future (meals are expanded into their ingredients), visible in the sidebar on desktop or via a cart icon in the header on mobile. Check items off as you shop — they dim in place; items from past days drop off automatically as today moves forward.
